...
//!
//! 问题（外部审查定点）：手写异步里 `let resp = http_post(url, body).await?` 一行，
//! 编译器替你拆状态机 + 建关联表；cell 模型里是"一个发请求、一个收响应"两个 step，
//! 关联表（哪个响应对应哪个请求）要自己在 State 里管——即"要手写多久 poll
//! 才有人封装好 epoll"这句话里的 poll。本模块把这一块做成类型化的、确定性可测的组件。
//!
//! ## 形态裁定：单槽同步 vs 乱序异步
//!
//! 仓库里的 [`ComposeLine`](composite::ComposeLine) 是单槽同步服务：`step(req)`
//! 立即产出 `resp`，同一时刻至多一个在途调用，关联退化为平凡——它不需要关联表，
//! 每会话一份 `Svc::State` + 一次 `poll_until(deadline)` 即可。这类形态配套的是
//! 会话模板（每会话 = 状态持有 + 期限轮询），demo 已用 [`Poller`](observe::ObservedPoller)
//! 表达。
//!
//! 本模块服务的是另一种形态：乱序/可变延迟的异步服务（网络对端、跨机管道——
...
//! 合法但别扭。故按讨论定论，关联调度器作为邻居组件存在：持有服务状态无关的
//! 关联结构，说 `PortCell` 这门货币语言（内部仍以原子值交换），但不把它当作 cell。
//!
//! ## 时间作值（诚实边界）
//!
//! 本调度器不读环境时钟：所有期限判定都经显式 `now: Instant` 输入注入。
//! 这与核心 `step` 纯函数的裁定一致（cell 内无时间，时间经泵喂入）。由此：
//! - 测试完全确定（注入假时钟，不依赖真实计时器粒度）；
...
/// [`drain_outcomes`](CallDispatch::drain_outcomes) 消费。每调用恰好产出一条判定
