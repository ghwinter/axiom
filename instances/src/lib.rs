...
//! > 下仅 `Carrier`（核心族，无条件）可解析。故上述 socket 以纯文本引用、不设
...
//! 门剖面（Kernel/Service）的注册白名单——本身就是部署侧选的；开放剖面（Tool/Embedded）里未注册
//! 第三方照常工作，选择权仍在用户，axiom 只提供"按剖面声明准入"的机制。生态引力风险（"原则上
//! 可替换、实际上无人替换"）的防御 = 两条既有纪律：载体目录呈现为菜单而非
...
#![forbid(unsafe_code)]
// no_std 说明（H1 审计修订）：本 crate 有意不参与 no_std 承诺——实例层依赖
