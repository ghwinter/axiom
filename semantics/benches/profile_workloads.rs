...
use axiom_semantics::movers::mailbox::BoundedMailbox;
use axiom_semantics::movers::ring::BoundedRing;
...
fn main() {
    ...
    timed("service: mailbox throughput", || {
        ...
        for i in 0..(ITERS / 2) {
            ...
            if let axiom_semantics::checks::delivery::Receipt::Item(v) = mb.try_recv() {
                acc ^= v;
            }
        }
        let _ = std::hint::black_box(acc);
