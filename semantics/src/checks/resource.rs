...
//!
//! 资源作为一等代数对象：交换幺半群 (M, ·, ε)。这是义务类资源轴（D1；
//! [`CarrierCost`] 的类/量级序）之外新增的可组合所有权维度——前四行为结构
//! （因果序/成本/容量/投递）描述单条接缝的运行幅度，资源描述跨拓扑的可组合
//! 所有权关系（frame 律）。
//!
//! **frame 律**：不相交所有权下的 `P · R` 可分别验证、组合 = 各分量之和
//! （"组合=真和"）。其前置"两份预算确实不相交"由 L2 单属主 在类型层背书——
//! `&mut` 独占保证同一时刻至多一个执行流触碰一块 `State`，这是 frame 推理合法
//! 化的地基。本模块不伪称能证明 `&mut` 不相交（那是借用检查器的职责），只背书
//! 一阶幺半群律与"组合 = 真和"。
//!
//! 诚实边界（A5）：本模块是知识单元（律 + 探针），非分配引擎——语义层不分配；
//! 具体聚合/归还（bring-up 时刻分配、运行期扩容）归实例层。此处确立的是资源量的
//! 组合律，不是资源物理来源。
...
/// "两份不相交预算的合并 = 各自之和"的真和——实现者须保证 `merge` 不为近似、
/// 不丢分量。
...
/// 记账侧的真和（[`FrameProbe::on_combine`] 后 `combined = left + right`）；
