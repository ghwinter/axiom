...
//!
//! 1. **饱和的数学面（2026-09-01 分析批，v1 规则集）**：三个开放项已裁决
//!    （完整论证见 `tmp/theory-notes/egraph-confluence-termination-2026-09-01.md`）：
//!    - **终止性**：重写关系不终止（结合律双向，节点数不变），但
//!      `EGraph::saturate` 必终止——"关系终止"与"算法终止"分离。
//!      论证：规则不创建原子、只创建 Chain 节点且保持去 Id 叶序，可达
//!      e-node 宇宙有界（引理 A）；每条规则遵守"仅对真实合并报进展"
//!      纪律，有进展轮严格递减类数（引理 B，monovariant）。轮数上界
//!      = 初始类数 − 1。停机即真不动点，非预算截断。
//!    - **合流性**：Newman 引理不适用亦不需要。正确命题"饱和至不动点
//!      = 输入事实 + 登记规则实例的最小同余闭包"成立（单调算子 +
//!      停机时全部一步结论在场 + 推导深度归纳），最小不动点唯一 ⟹
//!      判定与规则应用顺序无关。
//!    - **健全性**：≡_eg ⊆ ≡_beh 对 v1 规则集构造性成立——每条重写
//!      是 laws.rs oracle 已断言的行为律（结合律/单位律双面绿票），
//!      同余步由复合的行为函子性背书（迹展开等价）。反向包含不主张：
//!      ≡_eg ⊊ ≡_beh（未登记律：对称自然性、迹方程、余单位方程——
//!      依赖类型上下文，刻意不收录）。假阴性唯一来源 = 规则集不完备
//!      （登记边界），非机器缺陷；`max_rounds` 预算由此重定位为对未来
//!      规则集的工程保险（新规则若破坏"仅真实合并报进展"纪律，上述
//!      论证失效，预算接管）。
