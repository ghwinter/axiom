...
/// 部署期装配（模态③）：在装配点一次校验载体的成本预算，通过才返回驱动入口。
///
/// 这是模态③从"规格"到"机制"的接线：部署方在装配点显式选择预算，越界即装配失败
/// （返回 [`ContractError`](crate::checks::contract::ContractError)），而非驱动中静默运行。
/// 校验只发生在装配点（一次）；返回的入口即 [`drive_link`] 的函数指针（编译期验证 +
/// 零成本路径，不引入每消息税，也无闭包间接）。
pub fn assemble_link<A, B, C>(budget: CarrierCost) -> Result<Driver<A, B>, crate::checks::contract::ContractError>
where
    A: PortCell,
    B: PortCell<In = A::Out>,
    C: Carrier<A, B>,
{
    crate::checks::contract::validate_cost::<A, B, C>(budget)?;
    Ok(drive_link::<A, B, C>)
}
...
///
/// **成本标注（External 级）**：本边界是高成本接缝——反传播检查 + 状态不可信
/// （panic 后 `A::State`/`B::State` 可能半更新，属接缝责任，不由本函数修复）。
/// 热路径不付此税：工程约定"cell 内禁 panic"（失败必须是值，`Out = Result`；
...
/// 部署期装配（模态③）：有界接缝的合并校验——成本预算 和 容量（`CAP >= 1`）
/// 在装配点一次通过，返回驱动入口（同上：一次校验、热路径零税）。
///
/// 模态② 的编译期门（`assert_capacity_nonzero`）覆盖自带门的载体；本入口用
/// [`validate_seam`](crate::checks::contract::validate_seam) 在部署期承接无门载体的校验。
pub fn assemble_seam<A, B, C, const CAP: usize>(
    budget: CarrierCost,
) -> Result<Driver<A, B>, crate::checks::contract::ContractError>
where
    A: PortCell,
    B: PortCell<In = A::Out>,
    C: Carrier<A, B>,
{
    crate::checks::contract::validate_seam::<A, B, C, CAP>(budget)?;
    Ok(drive_link::<A, B, C>)
}

/// 无缓冲内联闭环驱动：`BODY -> FEED -> BODY` 经内联载体一拍（一次调用内两拍）。
///
/// **门禁**：要求 `FEED:` [`Moore`](crate::checks::contract::Moore)（部署者声明，模态④——仅声明、
/// 非证明：输出是否真的只依赖 `State` 是语义性质、不可判定（Rice），错误声明由声明者负责）。
/// 让渡合同（N1）：①让渡了什么——Moore 性质的机械见证（Rice 不可判定，①②③均不可达）；
/// ②为什么——违反后果（Mealy 误声明为 Moore → 输出序污染下游）虽不可逆，但无机械
/// 判据可落，定义面不可达；③代价由声明者全额承担（违约即声明欺诈）；边界测试——
/// 本入口对非 `Moore` FEED 拒绝汇编（模态①门），声明域止步于 FEED 一轴。
/// 有缓冲环（`BoundedCarrier`/队列）无需此门——缓冲即延迟（T3）。
///
/// > 与核心 [`Feedback`](axiom::cell_core::Feedback)`::step` 的关系待裁定：核心的单元
...
where
    BODY: PortCell,
    FEED: PortCell<In = BODY::Out, Out = BODY::In> + crate::checks::contract::Moore,
{
...
///
/// 闭合"错误 / 失败通路"（`foundations.md` §9.2）在物理侧的实现：用 `Out = Result`
...
/// 可失败链：把两个会失败的 cell（`Out = Result`）串成短路链，产出单层 `Result`。
///
/// 与 `drive_try` 的差异：`drive_try` 只要求 `A` 产生 `Result`、`B` 可非失败
/// （"OK 通道"专线，`Out = Result<B::Out, E>`）；`TryChain` 要求 `A` 与 `B` 都产生
...
/// 有界泵：把一组输入，经一个有界队列（容量 `CAP`）投到消费者 cell，返回其输出序列。
///
/// 这是 §9.1"有界/背压"的真实背压演示：生产端把 `A` 的输出投入容量 `CAP` 的有界
...
///
/// 这是"无界计数"的生成/物理侧：计数的多少由运行期序列（`IntoIterator`）决定，
...
mod no_std_tests {
    ...
    use crate::movers::carrier::InlineCarrier;
    use crate::movers::carrier::CarrierCost;
    use crate::drive::flow::{TryChain, drive_link, drive_seq};
    ...
    fn assemble_link_available_in_no_std() {
        let link: Result<_, crate::checks::contract::ContractError> =
            crate::drive::flow::assemble_link::<Inc, Double, InlineCarrier>(CarrierCost::ZeroAllocInline);
    ...
    fn drive_catch_contains_panic_and_passes_clean_drives() {
        ...
        assert!(crate::drive::flow::drive_catch::<PanicCell, Double>(&mut sa, &mut sb, -1).is_err());
        // 守约路径：正常通过。
        assert_eq!(crate::drive::flow::drive_catch::<PanicCell, Double>(&mut sa, &mut sb, 5), Ok(12));
    }
}
