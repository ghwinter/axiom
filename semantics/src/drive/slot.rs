...
//! 都合规）。本模块把"运行时选择一个合规居留项并驱动"做成安全的类型擦除：
//! 居留项的状态类型擦除为 `Box<dyn Any + Send>`，`step` 以函数指针保存
//! （针对该居留项单态化）。这是统一模型里"∃ 绑定"的物理侧——接口固定
...
//!   类型级拒绝（模态① 结构见证，零运行期检查）——落位律 A3 对"许可何时生效"
//!   的最强模态放置；
...
//!
//! **在途值归属（C5；热替换语义）**：换装可能丢弃旧居留项状态中未完成的工作
//! （在途值）。静默丢弃违反 A5——本模块提供两种显式形态：
//! - [`SlotDrive::swap`]：已知无在途的换装（调用方断言旧实现无未完成工作；
//!   违反 = 声明者责任）；
//! - [`SlotDrive::swap_and_drain`]：强制处置旧状态——旧状态连同其未完成工作
...
impl<I, O> SlotDrive<I, O> {
    ...
    ///
    /// **C5 语义边界**：本形态 = 已知无在途的换装——调用方断言旧居留项无未完成
...
/// 其状态仍可能持有在途（声明侧责任，A5），但静默丢弃在类型上不可能。
pub trait Drainable {
    /// 在途工作余量的类型。
    type Drain;
