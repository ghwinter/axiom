...
use axiom_semantics::movers::carrier::{Carrier, InlineCarrier, QueueCarrier};
...
fn inline_carrier_zero_alloc_semantics() {
    ...
    assert_eq!(out, 12);
    // 成本声明：零分配、内联。
    assert_eq!(<InlineCarrier as Carrier<Inc, Scaler>>::cost(), axiom_semantics::movers::carrier::CarrierCost::ZeroAllocInline);
}
...
fn queue_carrier_per_message_alloc_semantics_equal() {
    ...
    assert_eq!(
        ...
        axiom_semantics::movers::carrier::CarrierCost::PerMessageAlloc
    );
}
...
fn static_path_unrolls_declared_static_subgraph() {
    ...
    use axiom_semantics::drive::static_path::{run_declared_static, run_static};
...
fn spawned_flow_crosses_threads() {
    ...
    let out = axiom_semantics::movers::carrier::spawned_flow::<Inc, Scaler>(&mut sa, || (), 5);
    assert_eq!(out, 12);
}
...
fn spawned_flow_propagates_worker_panic() {
    ...
    let result = std::panic::catch_unwind(std::panic::AssertUnwindSafe(|| {
        axiom_semantics::movers::carrier::spawned_flow::<Inc, Boom>(&mut sa, || (), 5)
    }));
...
fn wire_macro_compile_time_inline() {
    ...
    let mut sa = ();
    let mut sb = ();
    let flow = axiom_semantics::wire!(Inc => Scaler);
