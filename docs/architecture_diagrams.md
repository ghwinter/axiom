...
graph TB
  subgraph Layer2["Layer 2: Machine (Computation)"]
    ...
    T["trait Machine {
      ...
      fn config_schema()
      fn init(ctx) -> State
      fn process(&mut State, ctx, Input) -> Self::ProcessOutput
                                 (SingleOutput | MultiOutput)
  ...
  subgraph Layer1["Layer 1: Ports (Communication)"]
    ...
    P4["MachineContext { lifecycle, signal, time, snapshot, initial_value }"]
  end
...
graph TB
  ...
  Spec --> Validate["validate() -- structural
  name uniqueness, endpoint existence, no self-loops

  validate_deep() -- requires PortSchema
  port/type/flow compat, resource budget,
  Moore cycle safety, degree constraints,
  Inline acyclicity (Kahn toposort)"]
