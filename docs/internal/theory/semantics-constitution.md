...
- **core（形状 / Shape）**：核心 `cell_core` 冻结。只声明因果数据流（`A.out → B.in`）的形状：类型、型位（typed hole）、三绑定态、T1 布线合法性。承载逻辑-D 结构不变量（L1 无静默丢失 / L2 单属主 / L3 容量归位 / L4 政策归驱动）。
...
**基座优先（substrate-first，平面区分口径）**。基座优先有两面，不可混谈：
- **契约面**：同步与异步是平级的两种等待模式（支点论）——契约本体不预设孰先孰后，等待点契约对两者同构表达。
- **物理面**：并发 / 异步是复杂软件的**生态默认剖面**——复杂软件本征地跑在多核多线程，模块分布多执行序是基例、非例外；单线程同步纯单元 = 该基座的退化极限（一个执行序、无交错、无等待），不是并发随后附加的原始态。无共享全局时钟时，不存在"普遍同步情形"可供一般化。

原口径"异步多线程 = 本体论默认"降格为"生态默认剖面"：默认属实例层选型与剖面推荐（Kernel/Embedded 偏同步、Service/Game 偏异步），不入契约本体。本层按物理面把异步 / 并发表述为默认剖面，同步纯形态按其退化极限表述。

**no_std = 抽象层纯承诺**。core 与 semantics 以 `no_std` 交付，是"零成本抽象"的另一面：形状层不依赖宿主运行时，是可移植的纯承诺（模态①的物理侧）。不把 no_std 当作终点或特权，只当抽象层交付形态的一个约束。
...
- **D1 义务类**：参数化谓词族 = 投递态（Delivered/Full/Closed 机械化 ②③；Timeout/Cancelled ④ 声明——L1 判别四态口径，delivery.rs）× 资源类（ZeroAllocInline/PerMessageAlloc/External）× 引用有效（代戳） × 生命周期（许可阶段）。
...
- **D6 语义 / Semantics**：从配置空间到可观察行为的映射 σ: W → Obs（boundary-ontology 定义 1.2），连同在该
  ...
  性质的真值不在任何 tₖ 被系统验证者只允许 ④（meta-foundations 定义 1.4）。**推论（语义不含物理发生）**：
...
- **D7 IO 二分**：axiom-IO = 信息传递（抽象层只断言"类型 + 因果"，机制与时机归物理载体，foundations §1.0a）；
  ...
  处理器 / 载体定义，不属 core。
- **D8 观测 / 观测层**：观测 = 读取可观察行为（σ 的余象 Obs 的成分）；观测不是第三种流类——观测值 / 控制值 /
...
- **D10 接口 / 组合不透明 / 并发 / 异步**：接口 = 一个节点暴露的、受类型的端口集合的可用子集；连接经线的合法性
  ...
  与事件 / 握手的原子性；它是基座，非退化附加。异步 = 可由挂起 / 恢复暂停的交互原子，是并发基座的可暂停
  实例。**边界（同步退化）**：单线程同步 = 该基座的退化极限；实现若以同步纯 cell 为默认原语、异步置于 feature
  门后（现状 `no_std` 单线程优先 + `async-seam` 门），记为基座优先的差距，非满足项。
- **D11 资源幺半群（State 上的实例代数；I1 裁决修正原"唯一真新增"标级）**：把资源携带到交换幺半群 $(M, \cdot, \varepsilon)$：$M$ = 全部可构筑资源，
  ...
  资源对象"。**边界（落于行为范畴，不改 core）**：资源幺半群是 State 上的实例代数（导出物——`State:
  Default + step` 已是全部承载机制，与 monoidal 同构族同格，律的审计对象，非词汇新概念；I1 元裁定 2026-09-01），
  不修改 core 的 `State: Default` 形状，是对 `State` 结构约束的扩充；与其 L2 单属主（`&mut` 独占）相容
  ——单属主是 frame 律所需的不相交所有权。**诚实边界**：这是语义语料中最大的一次扩展性整理（其余 D6–D10
  均为既有理论的重命名或补齐），但按 H2 封顶与 I1 裁决，它以实例代数身份存在，不以概念身份入词汇；其可靠
  依赖"资源不相交"假说，跨结点一致性不在此层。
...
### 编号命名空间总表 / Numbering-Namespace Master Table（编号治理）

> 语料共享字母表，但不共享命名空间：同一字母 "D" 被三套独立编号占用，各自有各自的
> 序号域。下表把散落的编号序列并列，作为命名空间治理总表——**明示各行是独立命名
> 空间，共享字母不等同于同序**。

| 序列 | 范围 | 出处 / 性质 |
|---|---|---|
| 定义集 **D1–D11** | 宪法 §2 定义集（义务类 / 模态 / 接缝 / 载体 / semantics / 语义 / IO 二分 / 观测 / step / 接口与并发异步 / 资源幺半群） | 宪法定义编号 |
| 听证 **D** | 第二次宪法修正的听证编号（修正 12.10） | 听证批次编号（≠ 定义集 D；见 monoidal.rs / laws.rs 注释、positioning §6.1、foundations T3、本卷 §3 / §12.3） |
| decision card **D**（D1 / D2 / D5 …） | 语义决策卡编号（async_seam 注释、vocabulary §11 锚点） | 决策卡编号（又一套） |
| 公理 **A1–A6** | 本卷 §1 | 公理编号 |
| 定理 **T1–T9** | foundations | 定理编号 |
| 契约 **R001 …** | evidence/contracts.md | 契约 / 裁决编号 |

**消歧判定律（disambiguation rule）**。凡引用共享字母的编号，须带限定词
（"定义 Dn" / "听证 D" / "decision card D"），将其指向唯一指称；不带限定词的裸 **D**
一律指向定义集 D1–D11。这呼应 semantic-vocabulary 的"歧义消解协议"——同文档内，
一个编号只允许一个指称（同文档唯一指称）。

**明示**：本表为命名空间治理，**不重编号、不新增概念**；只把已散落的编号序列并列
声明，供引用定位与歧义消解。
...
semantics/src/
  ...
  delivery.rs   投递判别四态：Delivered/Full/Closed 机械化（②③），Timeout/Cancelled 声明（④，机械化为物理选择）
...
> 命题（接缝契约论 / Seam-Contract Thesis）：与既有生态的兼容由接缝契约 + 义务声明 + 机制自由三者承担，不需牺牲拓扑或语义。
...
- **共形命题**：控制与观测都是值流 / 物理接口的形态，不为它们新增概念（§8.3）；
...
> 三案按序裁完：先立顶（H2），再立价（N1），再用顶与价判具体案（I1）。
> 产出 = 三条裁决文本 + 一条元裁定。全文与逐案证据见内部账本（gap-plan §0.15）。

### 12.1 H2 —— Schema 封顶条款

> **S 封顶于编译期可判定**。进入词汇表的每一概念，其判定负担必须落在编译期
> （模态①②）：不能给出编译期判据的性质，只能住声明面（④）或校验面（③），
> 不得以概念身份入库。实例代数、效应、时间一律不付"可判定性税"不入词汇。

H2 是 I1 的裁决依据，也是后续一切"要不要第六概念"问题的第一道闸。

### 12.2 N1 —— 可逆性分界函数（捆绑让渡书写纪律）

> **违反后果的可逆性**判据：违反后果不可逆（数据丢失、收敛破坏、守恒破坏、
> 因果不可重放）的性质 → **定义面**（类型/const，模态①②）；违反后果可逆
> （变慢、可重试、可重放）的性质 → **惯例面**（模态④，附让渡合同）。

捆绑条款（让渡书写纪律）：每个 ④ 声明域条目必须附**让渡合同**三字段——
...
> 物理平面（资源/效应/标注/环境）的入宪通道有且仅有三条——
> **State 内容**（作为实例代数的载体）、**基声明**（作为关于基的
> 侧条件）、**④标注**（附让渡合同）。没有第四条门。
