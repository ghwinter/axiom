...
**注记 4.6**。商类型在 Rust 的稳定类型系统中不可完备表达（第 5 节）；设备树以"列表 + 运行期校验"落实，是定理 5.1 中降级编码的实例。
