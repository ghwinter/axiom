...
## 9. 对 axiom 与 axiom-runtime 的应用 / Application to axiom and axiom-runtime
...
- **runtime = 义务代数的机械**：物理层自分层（boundary-ontology §9 在物理层内的递归应用）；义务类语法（投递态 × 资源类 × 引用有效 × 生命周期）为物理层的前四阶公理；`assemble_link`/`assemble_seam` 为装配校验（模态③ 机械）。
