...
> 三个案例库的分项分析见 docs/internal/{bevy-analysis,actix-analysis,modern-software-reference}.md
> （同为内部文档，未纳入本归档）

| 库 | 开放系统（细胞） | 布线 | 实例化宿主 | 组合策略 |
|---|---|---|---|---|
| bevy | System（输入 = SystemParam 声明的数据访问，输出 = 副作用） | 隐式：System 对 Component/Resource 的访问矩阵由调度器仲裁 | World（全局共享状态宿主） | Plugin 打包（system + 资源 + 调度） |
...
观察：
- bevy 的多对多是隐式布线——System 声明读/写哪些组件，调度器运行时仲裁；
  物理上做"零纯连接"（共享组件靠借用/锁仲裁，而非每条访问一条显式通道）。
- actix 把布线显式化为地址（N→1 点对点），mailbox 是实例化宿主。
- dsh 把布线做成事件的值语义（四种分发 = 布线的不同契约）。
...
3. **N2N 属于物理层**：多对多的并行调度、队列仲裁、借用、缓存、线程是 tokio /
   bevy-executor / actix 等物理实现的领域，axiom 不重造；axiom 提供载体契约与
