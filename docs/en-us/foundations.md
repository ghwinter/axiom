...
|---|---|---|
| **W1 (Composition)** | Systems can be composed via wiring into larger systems; composition is associative and unital; exchange holds via the half-braid slide (the `Slide`/`SlideInv` isomorphisms), with the Central Sliding side condition declared on the base (holding discard/copy ⟹ non-Cartesian base) | Open systems/operads |
...
**Formalization**: (c₁ ⊗ c₂) ⊗ c₃ ≅ c₁ ⊗ (c₂ ⊗ c₃) (`Assoc`/`AssocInv`); c ⊗ id ≅ c (`UnitR`/`UnitL` and inverses, unit object bound to `()`); c₁ ⊗ c₂ transposes to c₂ ⊗ c₁ via the half-braid (`Slide`/`SlideInv` — not the full symmetry σ; laws audited as behavioral-equivalence assertions, with the `Slide` laws holding under the Central Sliding side condition: holding ε/δ ⟹ non-Cartesian base, Benton–Hyland).
...
### T3. Well-Definedness of Cycles: an Abstraction-Layer Guard or a Physical-Layer Buffer, One of the Two (Self-Consistency Theorem · Second Amendment, Hearing D)
**Premises**: δ: S×Γ_in → S'×Γ_out; wiring consists of directed causal edges (A.out → B.in).
**Derivation (amended)**: the KPN proof (deterministic nodes + FIFO channels + blocking-read ⟹ the network is automatically well-defined and cycles are unremarkable) has its scope narrowed to buffered Kahn channels: buffering is delay, and a cycle implicitly acquires a one-beat guard through FIFO. A synchronous inline (zero-buffered, direct-call) cycle is not automatically well-defined—an immediate loop composes spin or same-beat composition explosion; "cycles are unremarkable" does not hold in the synchronous inline setting. The well-definedness obligation of a cycle must be borne by someone: the abstraction layer or the physical layer, one of the two—it cannot be pushed entirely onto the physics.
**Formalization**: abstraction layer G = (C, Γ, W), W = directed causal edges (no timing marks). The guard for cycle well-definedness comes from:
- **Abstraction-layer guard** (the ratified form of `Feedback`): the unit form fixes one inline closed iteration per external input, with a one-beat delay on the FEED side (Hearing D: guarded feedback; yanking—instant retrieval—is not its semantics; negative witness in laws.rs);
- **Physical-layer buffering**: carrier = channels/queues (Kahn-style, buffered) → the cycle implicitly acquires a one-beat guard via FIFO, zero abstraction constraints.
**Necessity**: necessary (well-definedness requires at least one step of delay around the loop—a structural fact of synchronous algebraic feedback; the KPN determinacy theorem is the buffered special case).
**Corollary**: the Moore guarantee and channel buffering are not "Delay declarations of the abstraction layer," but two means of guarding (state isolation / tick isolation); guard placement is decided by how the cycle is assembled, and the declaration obligation no longer defaults to the physics.

> **axiom realization**: `Feedback` expresses causal closure of cycles at the type layer and carries the one-beat guard built in (an abstraction-layer obligation); the buffered-cycle path's well-definedness is honored by the carrier (T6 cross-check). See `../en-us/core.md` and `../en-us/semantics.md`.
...
satisfying associativity, identity, and exchange via the half-braid slide:
> (c₁ ⊗ c₂) ⊗ c₃ ≅ c₁ ⊗ (c₂ ⊗ c₃);  c ⊗ id ≅ c;  c₁ ⊗ c₂ ≅ c₂ ⊗ c₁ (via `Slide`/`SlideInv`,
> Central Sliding side condition on the base)
...
- Feedback (`Feedback`) = a guarded extension of 3 (not a trivial "self-connection"
  instance — Hearing D ruling): compositionality comes from 3 (`BODY` composed with `FEED`),
  but its guard face — the cell form fixing one inline closed iteration per step plus a
  one-beat delay on the FEED side (the C2 ticking ruling), with yanking as an assertion
  object (negative witness: guarded feedback ≠ instant swap) — is a ruled structure not
  derivable from the other members; the well-definedness obligation is borne by that guard
  (T3 second amendment); the equivalent guard of the buffered-cycle path is physical;
