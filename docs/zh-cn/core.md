...
>
> **一句话**：axiom 核心层 = **compile-time DSL + 验证器**：全部"智能"（分析、
...
> axiom 核心层 = **compile-time DSL + 验证器**：全部"智能"在**编译期**耗尽；
> 产物是**普通 Rust 代码**。axiom 没有"运行时"——只有"编译期"与"编译后"两段。

**含义**：蓝图是**编译期构造物**（类型 + const + 宏生成的代码）。验证由宏 / 类型 /
...
如 `Conforms`、`CAP` 门、端口同构）；进入**宏 emit 检查**（`compile_error!`），当其可判定
