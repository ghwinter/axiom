...
**定理 15.3（动态税的不可消除部分）**  
> **2026-08 修订**：本定理原表述（"每消息 1 次堆分配 + 1 次动态分派 + 1 次
> 字符串比较，数学上必然"）经实测**不准确**：动态税的真实构成是**每级 2 次
> `Box` 堆分配**（改造后每级 ~1 次，见下），且"不可避免"的部分与实现
> 冗余混为一谈。以下为修订后的准确表述。

若拓扑在运行时方可确定，则**安全 Rust** 下每条消息每级至少需要：
- **1 次堆分配**：值必须跨异构类型传递（`Out → Box<dyn Any> → In`），
  `Box<dyn Any>` 是安全 Rust 的类型擦除唯一方式（`forbid(unsafe_code)`
  禁止 `transmute` 型零分配类型转换）。`from_port_name` 解包输入 Box +
  输出 `into_any` 重新装箱 = **净 1 次/级**；
- **1 次动态分派**：机器的 `process` 经 `dyn RunningMachine` 虚调用。

即 $\text{deploy\_dynamic}$ 不可能是零成本的——**但"不可避免"仅指上述
安全 Rust 下限（每级 ~1 次分配 + 虚调用）**，不是早期实现的每级 2 次分配。

> **实测（2026-08，3 级链，100k 消息，release + 分配计数器）**：
> - 改造前（每级 2 次装箱）：**6.0 allocs/msg**，动态税 ~400×；
> - ID 化 + `inject` 免冗余装箱：非融合 **3.0**、融合 **4.0** allocs/msg；
> - **unsafe 破局（封装点 `typed_slot`，`design-principles.md` §5.5）**：
>   融合链 **1.0 allocs/msg**（外部输入 1 次，同类型级间 0 分配）、时间
>   36→27ms——分配税消除，剩余为动态分派 + 类型擦除机制成本；
> - 静态路径（`FlowThrough` 线性流式）：**0.000 allocs/msg**。
>
> "级间 0 分配"的安全 Rust 尝试（`ScratchMachine`/`Unpack` 类型化单槽）
> 曾证伪：单槽 `take()` 后槽空必重装箱；运行时 `TypeId` 相等无法做类型化
> 写入。**经 `design-principles.md` §5.5 分层决策**（core `forbid`、runtime
> 允许封装性 unsafe），`typed_slot` 用 `TypeId` 检查 + 位拷贝（`ptr::read`/
> `copy_nonoverlapping`）达成同类型级间 0 分配——动态路径的分配税降至
> 外部输入 1 次，**0 分配仍仅静态路径可达**（编译期类型化）。

> **推论 15.3a（动态路径的正当性）**  
> 动态税（每级 ~1 次分配 + 虚调用）是**运行时拓扑灵活性**的对价。当且仅当
> 拓扑必须由配置、插件或运行时决策决定时，支付此税是合理的；否则应使用
> 静态路径（0 分配）。动态税随级数线性增长——**深链请优先静态路径**。
