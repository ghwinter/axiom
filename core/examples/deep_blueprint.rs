...
//! 编译成本数据）。运行期零成本由核心的 [`chain` bench](../benches/chain.rs) 聚焦测量
//!（本探针不重复）：`step` 全类型参数化、组合缝零指令。本探针补它没有的编译侧与单态化
//! 体积——跌破"零成本"被误读成"编译也零成本"的坑：运行期零，编译期不零，且随深度涨。
//!
//! ## 方法学（诚实测量，随本文件行走）
//!
//! - **测量分离**：先把 `axiom` crate 本体热缓存建好，再 `time -p cargo build --release
//!   --bin deep_blueprint` 取 `real`。想隔离本文件边际编译成本，先用一个浅测一次、再
//!   在此文件加深一层重测，取两次差值。
...
//! - **诚实边界（A5）**：以下为本机一次测得，非扇区，随 rustc / LTO / 机器漂移；二进制
//!   体积依赖 `lto = true`。不伪称这些数字是任何机器的常数——只给一张"长什么样"的快照
//!   ＋你可在此复跑的探针。
//!
