...
//!
//! 层界注记：四构件是**形状层**清单；激活（等待点/运行期绑定）属语义层，
//! 全谱概念为五（§0.8 供给边界）。
//!
...
/// > 生成/递归层面的运行期实例网（由 semantics/载体驱动）。本构造子表达"种类封闭、
...
/// ∃（运行期绑定）在定义侧的锚点；具体的运行期存在化填充由 semantics/载体承担。
...
/// 断言一个居留项/线可放入给定类型化位置（编译期）；不满足 T1 则该 impl 不存在 → 编译失败。
...
mod tests {
    ...
    fn slot_declares_interface_and_conforms() {
        // 型位定义是可编译的、零大小的（∃ 绑定在定义侧）。
        let _ = Slot::<i32, i32>::declare();
        // 型位的接口（I=O=i32）可被任意符合该对偶的居留项参数化地填入（T1）。
        type S = Slot<i32, i32>;
        assert_conforms::<S, Inc>();
        // Rep 也是 In=Out=i32 → 仍可填充同一型位（未来居留项的存在量化）。
