...
// ── FuncRef: borrow-input extension (A4) ─────────────────────────────────────
// ════════════════════════════════════════════════════════════════════════

/// A `Func` that can produce its output from a **borrowed** input
/// (zero-copy extension for fused pipelines).
///
/// # Why this exists
///
/// `Func::call` takes `Self::Input` by value (`Input: Send + 'static` — there
/// is no `&str` path): every invocation must *own* its input. For REPL lines,
/// large buffers, or hot pipelines this forces a copy per call even when the
/// function only reads the input. `FuncRef` explicitly declares that an
/// implementation can build its output from a borrow, so a fused pipeline
/// (a synchronous call chain) can drive that step through `call_ref` and
/// eliminate the input copy.
///
/// # Contract
///
/// 1. `call_ref(&input)` must **not** mutate `input`, and must produce the
///    same output as `call` for the same value (purity is preserved).
/// 2. If an implementation cannot borrow (its output must own part of the
///    input), it simply does **not** implement `FuncRef` — a fused pipeline
///    falls back to `Func::call` (owned path) for non-`FuncRef` steps.
/// 3. Orthogonal to `FuncWithScratch`: `FuncRef` removes the *input copy*,
///    `FuncWithScratch` removes *intermediate allocations*; a `Func` may
///    implement both.
///
/// # Usage in a fused pipeline
///
/// ```ignore
/// // A fused chain prefers the borrow path for steps that declare it:
/// fn fused2<A, B>(input: &A::Input) -> B::Output
/// where
///     A: FuncRef<Output = B::Input>,
///     B: FuncRef,
/// {
///     B::call_ref(&A::call_ref(input))
/// }
/// ```
pub trait FuncRef: Func {
    /// Produce the output from a borrowed input. Must be side-effect-free
    /// with respect to `input` and agree with `Func::call` on the value.
    fn call_ref(input: &Self::Input) -> Self::Output;
}
...
pub struct Scratched<F: FuncWithScratch>(core::marker::PhantomData<F>);
...
/// ```
pub struct FuncScratchPipeline<Steps>(core::marker::PhantomData<Steps>);
