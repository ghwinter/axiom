...
impl<I: Send + Sync + Clone + 'static> Machine for Collector<I> {
    ...
    fn process(state: &mut Vec<I>, _ctx: &MachineContext, input: CollectorInput<I>) -> SingleOutput<CollectorOutput<I>> {
        match input {
            CollectorInput::Input(v) => {
                state.push(v);
                // Yield the full snapshot on the observe port.
                SingleOutput::Yield(CollectorOutput::Snapshots(state.clone()))
            }
        }
    }
