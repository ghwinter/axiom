...
impl<I: Send + Sync + Clone + 'static> Machine for Identity<I> {
    ...
    fn process(_state: &mut (), _ctx: &MachineContext, input: IdentityInput<I>) -> SingleOutput<IdentityOutput<I>> {
        match input {
            IdentityInput::Input(v) => SingleOutput::Yield(IdentityOutput::Output(v)),
        }
    }
