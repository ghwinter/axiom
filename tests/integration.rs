...
impl Machine for SimpleCounter {
    ...
    fn process(
        ...
        state.count += v;
        SingleOutput::Yield(CounterOutput::out(state.count))
    }
...
fn test_machine_process() {
    ...
    assert_eq!(r1, SingleOutput::Yield(CounterOutput::out(5)));

    let r2 = SimpleCounter::process(&mut state, &ctx, CounterInput::in_(3));
    assert_eq!(r2, SingleOutput::Yield(CounterOutput::out(8)));
}
...
fn test_machine_full_lifecycle() {
    ...
    assert_eq!(result, SingleOutput::Yield(CounterOutput::out(60)));
...
fn test_machine_process_idle() {
    ...
    impl Machine for Idler {
        ...
        fn process(_: &mut (), _: &MachineContext, _: IdlerInput) -> SingleOutput<IdlerOutput> {
            SingleOutput::Idle
        }
        fn cleanup(_: (), _: &MachineContext) -> Result<(), CleanupError> { Ok(()) }
    }
    ...
    assert!(matches!(result, SingleOutput::Idle));
}
...
fn test_machine_process_done() {
    ...
    impl Machine for OneShot {
        ...
        fn process(state: &mut bool, _: &MachineContext, input: OneShotInput) -> SingleOutput<OneShotOutput> {
            let v = match input { OneShotInput::in_(v) => v };
            if *state {
                SingleOutput::Done
            } else {
                *state = true;
                SingleOutput::Yield(OneShotOutput::out(v))
            }
        }
    ...
    assert_eq!(r1, SingleOutput::Yield(OneShotOutput::out(99)));

    let r2 = OneShot::process(&mut state, &ctx, OneShotInput::in_(100));
    assert!(matches!(r2, SingleOutput::Done));
}
...
impl Machine for Doubler {
    ...
    fn config_schema() -> ConfigSchema { ConfigSchema::new() }
    fn init(_ctx: &MachineContext) -> Result<(), InitError> { Ok(()) }
    fn process(_: &mut (), _: &MachineContext, input: In<i32>) -> SingleOutput<Out<i32>> {
        SingleOutput::Yield(Out(input.0 * 2))
    }
    fn cleanup(_: (), _: &MachineContext) -> Result<(), CleanupError> { Ok(()) }
    fn deterministic() -> bool { true }
}
...
fn test_single_port_convenience() {
    ...
    assert_eq!(r, SingleOutput::Yield(Out(42)));
}
